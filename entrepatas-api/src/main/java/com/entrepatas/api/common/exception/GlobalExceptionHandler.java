...
public class GlobalExceptionHandler {
        ...
        public ResponseEntity<Map<String, String>> handleValidationErrors(
                        MethodArgumentNotValidException ex) { // este metodo se llama cuando el body no cumple las
                                                              // anotaciones
                                                              // (@NotNull, @NotBlack, etc)
                Map<String, String> errors = new HashMap<>();

                ex.getBindingResult().getFieldErrors() // trae el resuletado del binding. o sea el proceso de convertir
                                                       // json ->
                                                       // objeto
        ...
        public ResponseEntity<Map<String, String>> handleIllegalArgument( // RespondeEntity permite controlar status
                                                                          // code
                                                                          // (400, 404, etc) body json
                                                                          // Map<String, String> devuelve un json
                                                                          // {"name":
                                                                          // "name es
                        // requerido", "specie": "specie es requerido"}
                        IllegalArgumentException ex) { // suele ocurrir si se hace PetStatus.valueOf("asdf").. esto
                                                       // revienta la
